...
Демо открывается одинаково во всех современных браузерах. Caddy на сервере
настроен на HTTP/1.1 — это обход проблемы с разрывами idle HTTP/2-соединений
в РФ-сетях, из-за которой ранее Chromium-браузеры получали 10+ секундные
задержки на повторных запросах. Подробности — в `deploy/README.md`.
...
Для развёртывания на VPS используются скрипты из папки `deploy/`. Полная пошаговая инструкция, включая раздел «Известные особенности и подводные камни» (Docker Hub rate limit, настройка override-юнита Caddy, HTTP/2 ping-failed в РФ-сетях, нюансы pytest и пр.) — в [deploy/README.md](deploy/README.md).
