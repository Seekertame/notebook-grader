...
### HTTP/2 ping-failed на idle-соединениях в РФ-сетях

При использовании HTTP/2 (`protocols h1 h2` в Caddyfile) Chromium-браузеры
из РФ-сетей периодически получали 10+ секундные задержки на повторных
запросах к приложению, в том числе каскадные на 30 и более секунд.

Диагностика через `chrome://net-export/` показала следующий сценарий:

1. Запрос отправлен по уже установленному HTTP/2-стриму (3 мс с начала).
2. Браузер ждёт ответа.
3. По дороге между Chrome и Caddy idle TCP-сокет молча отбрасывается
   (NAT/firewall провайдера или хостинга, без RST-пакета).
4. Через ~10 секунд Chrome не получает ответа на HTTP/2 PING-фрейм,
   логирует `ERR_HTTP2_PING_FAILED` с `description='Abandoned.'`,
   рвёт соединение и переоткрывает новое.
5. На новом соединении запрос проходит за обычные ~300 мс.
6. Если разрыв случается каскадно, набегает 30+ секунд или больше.

При этом серверные замеры (`journalctl -u notebook-grader`) показывают,
что uvicorn отдаёт ответ за миллисекунды — задержка не в приложении.

Решение — отключить HTTP/2 в Caddy, оставив только HTTP/1.1. В HTTP/1.1
нет мультиплексирования и PING-фреймов: если idle-сокет молча умер,
браузер откроет новое соединение при следующем запросе без многосекундной
паузы. Глобальный блок Caddyfile:
...
После применения многосекундные «зависания» в Chromium-браузерах из
РФ-сетей пропадают. Замедления от потери HTTP/2-мультиплексирования на
этом приложении незаметно — фронт лёгкий, параллельных запросов мало.